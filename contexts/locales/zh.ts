export const zh = {
    // Sidebar
    newChat: '新对话',
    searchHistory: '搜索历史...',
    history: '历史记录',
    newFolder: '新建文件夹',
    collapseSidebar: '收起侧边栏',
    expandSidebar: '展开侧边栏',
    // Personas
    personas: '角色',
    selectPersona: '选择一个角色',
    createPersona: '创建角色',
    startChat: '开始聊天',
    editPersona: '编辑角色',
    deletePersona: '删除角色',
    // Persona Editor
    personaEditor: '角色编辑器',
    personaName: '名称',
    personaNamePlaceholder: '例如，杰克船长',
    personaAvatar: '头像',
    personaAvatarDesc: '输入一个表情符号或图片网址。',
    personaBio: '简介',
    personaBioPlaceholder: '对该角色的简短描述。',
    personaSystemPrompt: '系统提示',
    personaSystemPromptPlaceholder: '定义角色的性格和指示...',
    personaTools: '工具',
    savePersona: '保存角色',
    builderInputPlaceholder: '例如，“把它变成一个乐于助人的图书管理员”',
    builderApplying: '正在应用更改...',
    // Welcome
    welcomeTo: '欢迎来到 KChat',
    welcomeDesc: '在下方选择一个模型并开始新的对话。',
    // Chat Input
    typeMessage: '输入您的消息...',
    tools: '工具',
    codeExecution: '代码执行',
    googleSearch: '谷歌搜索',
    urlContext: '网页上下文',
    urlContextPlaceholder: '输入URL以提供上下文',
    showThoughts: '显示思考过程',
    showThoughtsDesc: '在最终答案之前显示模型的思考过程。',
    thoughts: '思考过程',
    studyLearn: '学习模式',
    studyLearnDesc: '为此对话启用持久的学习助手。',
    // Translator
    translate: '翻译',
    translator: '翻译器',
    sourceLanguage: '源语言',
    targetLanguage: '目标语言',
    autoDetect: '自动检测',
    natural: '自然',
    literal: '直译',
    translationHistory: '翻译历史',
    enterText: '输入要翻译的文本...',
    translating: '翻译中...',
    paste: '粘贴',
    clear: '清空',
    read: '朗读',
    copy: '复制',
    swapLanguages: '交换语言',
    dropFilesHere: '拖拽到此处上传',
    // Modals
    editChat: '编辑对话',
    shareChat: '分享对话',
    deleteChatConfirm: '您确定要删除此对话吗？',
    iconEmoji: '图标 (Emoji)',
    title: '标题',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    newFolderName: '新文件夹',
    editFolderName: '编辑文件夹',
    folderName: '文件夹名称',
    // Settings
    settings: '设置',
    general: '通用',
    language: '语言',
    languageDesc: '设置应用的显示语言。',
    english: '英语',
    chinese: '中文',
    appearance: '外观',
    theme: '主题',
    themeDesc: '切换浅色和深色模式。',
    light: '浅色',
    dark: '深色',
    behavior: '行为',
    advanced: '高级',
    model: '模型',
    apiKeyEnvVar: '密钥已由环境变量设置',
    suggestions: '建议回复',
    suggestionsDesc: '显示 AI 生成的回复建议。',
    suggestionModel: '建议模型',
    suggestionModelDesc: '用于生成回复建议的模型。',
    langDetectModel: '语言检测模型',
    langDetectModelDesc: '用于翻译器中自动检测语言的模型。',
    autoTitleGeneration: '自动生成聊天标题',
    autoTitleGenerationDesc: '为新聊天自动创建标题和图标。',
    titleGenModel: '标题生成模型',
    titleGenModelDesc: '用于生成标题的模型。',
    defaultSearch: '默认启用谷歌搜索',
    defaultSearchDesc: '为新对话默认启用谷歌搜索。',
    useSearchOptimizerPrompt: '使用优化搜索提示',
    useSearchOptimizerPromptDesc: '使用专门的提示来增强搜索查询，以获得更好的结果。',
    globalSystemPrompt: '全局系统提示',
    globalSystemPromptDesc: '一个附加到所有对话和角色前的提示。',
    optimizeFormatting: '优化格式',
    optimizeFormattingDesc: '添加指令以改善回复的结构和可读性。',
    thinkDeeper: '深度思考',
    thinkDeeperDesc: '添加指令以进行更全面、更深入的推理。',
    dataManagement: '数据管理',
    importData: '导入数据',
    importDataDesc: '从 JSON 文件导入对话和设置。',
    exportSettings: '导出设置',
    exportSettingsDesc: '将应用设置导出为 JSON 文件。',
    exportData: '导出所有数据',
    exportDataDesc: '导出所有对话、文件夹和设置。',
    clearHistory: '清除所有数据',
    clearHistoryDesc: '永久删除所有对话、文件夹和设置。',
    clearHistoryConfirm: '您确定要删除所有数据吗？此操作无法撤销。',
    // Archive
    archive: '归档',
    unarchive: '取消归档',
    archivedChats: '已归档对话',
    archiveDesc: '查看和管理已归档的对话。',
    enableCustomApiSettings: '启用自定义 API 设置',
    enableCustomApiSettingsDesc: '使用您自己的 API 密钥和基础 URL，而不是默认设置。',
    customApiKey: '自定义 API 密钥',
    customApiKeyDesc: '输入您的自定义 API 密钥。',
    customApiKeyPlaceholder: '输入您的 API 密钥',
    customApiBaseUrl: '自定义 API 基础 URL',
    customApiBaseUrlDesc: '输入您的自定义 API 基础 URL。',
    customApiBaseUrlPlaceholder: '输入您的 API 基础 URL',
};